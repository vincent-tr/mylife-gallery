...
const logger = createLogger('mylife:gallery:sync:images');

async function fsList(album) {
  const list = [];
  const albumDirectory = path.join(getConfig('gallery'), album.name);
...
async function fsListDir(list, directory) {
  for(const item of await directoryList(directory)) {
    ...
    if(stat.isFile() && ext && supportedExtensions.has(ext.toLowerCase())) {
      list.push(path.relative(getConfig('gallery'), item));
      continue;
    }
...
exports.delete = async function delete_(album, ipath) {
  logger.info(`Delete image '${ipath}' of album '${album.name}'`);
...
exports.create = async function create(album, ipath) {
  logger.info(`Create image '${ipath}' of album '${album.name}'`);

  const result = await load(path.join(getConfig('gallery'), ipath));
  if(!result) {
    // loading failed
    return;
  }
  ...
  const nodes = ipath.split('/').filter(node => node);
  nodes.pop(); // last is image name
  nodes.shift(); // first is album
  if(nodes.length) {
    metadata.tags.push(... nodes);
  }

  const { name } = path.parse(ipath);
